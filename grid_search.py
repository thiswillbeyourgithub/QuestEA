    ...
    **kwargs,
    ...
    param_grid = ParameterGrid(
        {
            ...
            "cluster_method": [
                "kmeans"
            ],  # "spectralcosine"],  # "bisectingkmeans"],  # kmeans, SpectralCosine
            "dimred_method": [
                "pca",
                "umap",
                "pacmap",
    ...
    for datasetname in tqdm(dataset_list, desc="dataset", colour="magenta"):
        ...
        for param in tqdm(param_grid, desc=datasetname, smoothing=0, colour="magenta"):
            ...
            try:
                output = QuestEA(datasetname=datasetname, **param).output
                ...
                metadata = []
                metadata_headers = ["subject_id", "mode", "norm", "dimred_method"] + [
                    f"pred K={k}" for k in output["n_cluster"]
                ]
                ...
                if output["sentence_embeddings"] is not None:
                    metadata = []
                    metadata_headers = ["question_id", "mode", "norm", "dimred_method"]
                ...
                for it in comp["f1"].keys():
                    writer.add_hparams(
                        ...
                        },
                        {
                            ...
                            "perf_randindex_raw_2": comp["randindex"][it]
                            - comp["randindex_vs_randint"][it],
                        },
                    )
                writer.add_scalar("perf_f1_mean" + tag, comp["perf_f1_mean"])
                writer.add_scalar(
                    "perf_randindex_mean" + tag, comp["perf_randindex_mean"]
                )
